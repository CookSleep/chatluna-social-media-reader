...
function createMediaNote(targets: string[]) {
    ...
    const targetText = uniqueTargets.length
        ? `If the user's task is related to the media content, you MUST use the \`read_files\` tool to directly read the media URLs from ${joinWithAnd(uniqueTargets)} to obtain specific information, without worrying about factors such as duration or size. Ignore this requirement if you do not have the \`read_files\` tool.`
        : `If the user's task is related to the media content, you MUST use the \`read_files\` tool to directly read the media URLs to obtain specific information, without worrying about factors such as duration or size. Ignore this requirement if you do not have the \`read_files\` tool.`
